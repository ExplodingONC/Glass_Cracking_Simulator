...
#ifndef USER_IMU_MPU6050_H_
#define USER_IMU_MPU6050_H_
...
u8 IMU_Init(void);                                 //初始化MPU6050
u8 IMU_WriteLen(u8 addr, u8 reg, u8 len, u8 *buf); // IIC连续写
u8 IMU_ReadLen(u8 addr, u8 reg, u8 len, u8 *buf);  // IIC连续读
u8 IMU_WriteByte(u8 reg, u8 data);                 // IIC写一个字节
u8 IMU_ReadByte(u8 reg);                           // IIC读一个字节

u8 IMU_SetGyroFsr(u8 fsr);
u8 IMU_SetAccelFsr(u8 fsr);
u8 IMU_SetLPF(u16 lpf);
u8 IMU_SetRate(u16 rate);
u8 IMU_SetFifo(u8 sens);
